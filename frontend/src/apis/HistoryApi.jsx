...
export const requestPages = async (isHistory=true, pageNum) => {
  const URL = isHistory ? `history/?page=${pageNum}` : `history/bookmarked/?page=${pageNum}`
  console.log(URL)
  const res = await customAxios.get(URL, {headers: {'X-CSRFToken': csrftoken}});
  return res.data;
}

export const requestDeleteHistory = async (historyId) => {
  const res = await customAxios.delete(`history/${historyId}/`, {headers: {'X-CSRFToken': csrftoken}});
  return res.data;
}

export const requestHistoryLog = async (historyId) => {
  const res = await customAxios.get(`history/${historyId}/`, {headers: {'X-CSRFToken': csrftoken}});
  return res.data;
}
